...
class MainWindow(QMainWindow):
    ...
    def copy_clicked(self):
        ...
        """
        src_path = self.ui.lePath.text().strip()   # источник: файл ИЛИ папка
        dest_dir = self.ui.leDest.text().strip()   # директория назначения (может быть пустой)

        if not src_path:
            QMessageBox.information(self, "Копирование", "Выберите источник (файл или папку).")
            return

        try:
            new_path = FileManager.copy_file(src_path, dest_dir or None)
            self.ui.txtLog.append(f"✓ Скопировано в: {new_path}")
            QMessageBox.information(self, "Готово", f"Скопировано в:\n{new_path}")
    ...
    def pick_dest_dir(self):
        ...
        path = QFileDialog.getExistingDirectory(self, "Выбрать директорию назначения")
        if path:
...
if __name__ == "__main__":
    ...
    w = MainWindow()
    w.setWindowTitle("FileManager — копирование (демо)")
    w.resize(900, 600)
    w.show()
    sys.exit(app.exec_())
